...
On a machine with installed Chrome and Edge, `npx playwright test --config=playwright.product.config.js` runs the full product-browser suite; `npm run test:release -- --require-products` requires both product browsers. The release script builds twice in a clean source copy, compares six generated-file hashes, independently packs and installs two exact nine-file archives, compares all packed files and archive bytes, and runs the same public contract from source and installed package at root and nested paths. Repeatability on one pinned toolchain does not establish reproducibility across machines or prove the WASM binary's source provenance.
...
The package's project [LICENSE](LICENSE) contains GNU AGPL version 3 text; `package.json` deliberately says `SEE LICENSE IN LICENSE` until the project's exact `AGPL-3.0-only` or `AGPL-3.0-or-later` designation and copyright attribution are confirmed. The browser Worker bundles MIT-licensed `quickjs-emscripten-core`, `@jitl/quickjs-wasmfile-release-sync`, and `@jitl/quickjs-ffi-types` 0.32.0. The WASM uses vendored MIT-licensed QuickJS 2025-09-13 with an upstream Emscripten-specific patch; [the pinned upstream build recipe](https://github.com/justjake/quickjs-emscripten/blob/df4efb9ef2cb25c417ecb57986da462d11b244ed/packages/variant-quickjs-wasmfile-release-sync/Makefile) specifies Emscripten 5.0.1. `dist/THIRD_PARTY_NOTICES.txt` identifies verified upstream notices and the exact WASM hash. The linked Emscripten/libc/compiler inputs and any additional notice or source obligations are not yet established. The [source repository](https://github.com/tarun-choudhary-dev/javascript) is public, but no release tag or corresponding-source bundle for this private candidate has been finalized. Do not treat the current package as cleared for public distribution.
